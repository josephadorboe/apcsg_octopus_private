<?php
	...
	$actionmodel = htmlspecialchars(isset($_POST['actionmodel']) ? $_POST['actionmodel'] : '');
	$dept = 'OPD';
	...
	switch ($actionmodel)
	{
            ...
            if ($preventduplicate == '1') {
                if (empty($patientcode)) {
                    ...
                    $actions = "consultationpatientaction?'.$idvalue";
                        ...
                        $patientaction = 1;
                        $sqlresults = $visitactioncontroller->endconsultation($patientcode,$days,$visitcode,$consultationcode,$patientaction,$servicerequestedcode,$action,$currentusercode,$currentuser,$instcode,$outcometable,$patientconsultationsarchivetable,$patientconsultationstable,$patientvisittable,$Prescriptionstable,$patientsInvestigationRequesttable,$patientproceduretable,$patientsDevicestable,$patientbillitemtable); 
                        $action = "End Consultation";
                       // $actions = "consultationpatientaction?'.$idvalue";
                            if($sqlresults == '2'){
        ...
        // 24 oct 2023, 21 APR 2022 , 28 MAY 2021
		case 'patientsaction':
			$consultationcode = htmlspecialchars(isset($_POST['consultationcode']) ? $_POST['consultationcode'] : '');
			$patientaction = htmlspecialchars(isset($_POST['patientaction']) ? $_POST['patientaction'] : '');
			$patientdate = htmlspecialchars(isset($_POST['patientdate']) ? $_POST['patientdate'] : '');
            ...
            if ($preventduplicate == '1') {
                if (empty($consultationcode) || empty($cancelreason)) {
                    ...
                    $actions = "consultationpatientaction?'.$idvalue";
                        ...
                        $sqlresults = $visitactioncontroller->cancelconsultation($consultationcode,$visitcode,$cancelreason,$days,$currentusercode,$currentuser,$instcode,$patientconsultationsarchivetable,$patientconsultationstable,$patientvisittable,$Prescriptionstable,$patientsDevicestable,$patientsInvestigationRequesttable,$patientproceduretable,$patientcomplainstable,$diagnosistable,$patientmanagementtable,$patientsnotestable,$patientsoxygentable,$physicalexamstable,$patientsServiceRequesttable,$patientvitalstable); 
            ...
            if ($preventduplicate == '1') {
                if (empty($consultationcode) || empty($physician)) {
                    ...
                    $actions = "consultationpatientaction?'.$idvalue";
                        ...
                        $physicancode = $sp['0'];
                        $physicaname = $sp['1'];
                        $sqlresults = $patientconsultationstable->reassignpatients($days,$physicancode,$physicaname,$consultationcode,$patientcode,$instcode);  
                        $action = "Reassign Patient to";
                        if($sqlresults == '2'){
                            $actions = 'patientconsultationgp';
                        }else{
                            $actions = "consultationpatientaction?'.$idvalue";
                        }                       
                        $result = $engine->getresults($sqlresults,$item=$physicaname,$action);
                        $re = explode(',', $result);
                        $status = $re[0];					
                        $msg = $re[1];
                        $event= "$action: $form_key $msg";
