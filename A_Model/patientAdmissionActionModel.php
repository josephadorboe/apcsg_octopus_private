<?php
	...
	$admissionactionmodel = isset($_POST['admissionactionmodel']) ? $_POST['admissionactionmodel'] : '';
	$dept = 'OPD';

	Global $instcode;
	
	// 26 FEB 2022  
	switch ($admissionactionmodel)
	{
		...
		case 'patientsaction':
            ...
            if ($preventduplicate == '1') {
                if (empty($patientaction)) {
                    ...
                    if ($patientaction == '10') {
                        $action = 'DISCHARGED'; 
                        $acts = $admissionactionsql->update_patientaction_dischargeonly($visitcode, $consultationcode, $days, $patientaction, $action, $remarks);               
                        ...
                        if (empty($referalservices) || empty($physician) || empty($referaltype) || empty($remarks) ) {
                            ...
                            $acts = $referalsql->update_patientaction_referal($form_key, $patientcode, $patientnumber, $patient, $visitcode, $day, $consultationcode, $days, $patientaction, $action, $remarks, $servicecode, $servicesed, $reviewnumber,$requestcode, $age,$gender, $paymentmethod,$paymentmethodcode,$schemecode,$scheme,$consultationpaymenttype,$physiciancode,$physicians,$referaltype,$consultationnumber,$referalserviceamount,$currentshiftcode,$currentshift,$currentusercode, $currentuser, $instcode,$currentday,$currentmonth,$currentyear);
                        ...
                        if (empty($patientdeathdate) || empty($patientdeathtime) || empty($deathremarks)) {
                            ...
                            $acts = $admissionactionsql->update_patientaction_death($patientcode, $patientnumber, $patient, $visitcode, $consultationcode, $days, $patientaction, $action, $remarks, $patientdeathdate,$patientdeathtime,$deathremarks,$deathrequestcode, $age,$gender,$currentusercode, $currentuser, $instcode,$currentday,$currentmonth,$currentyear);
                    ...
                    if (empty($patientdate) || empty($services) || empty($remarks)) {
                        ...
                        $acts = $admissionactionsql->update_patientaction_followup($form_key, $patientcode, $patientnumber, $patient, $visitcode, $patientdate, $consultationcode, $days, $patientaction, $action, $remarks, $servicecode, $servicesed, $requestcodecode,$currentusercode, $currentuser, $instcode,$currentday,$currentmonth,$currentyear);
                    ...
                    if (empty($patientdate) || empty($services) || empty($remarks)) {
                        ...
                        $acts = $admissionactionsql->update_patientaction_review($form_key, $patientcode, $patientnumber, $patient, $visitcode, $patientdate, $consultationcode, $days, $patientaction, $action, $remarks, $servicecode, $servicesed, $requestcodecode,$currentusercode, $currentuser, $instcode,$currentday,$currentmonth,$currentyear);
