<?php
	...
	$setupprocedurepricemodel = htmlspecialchars(isset($_POST['setupprocedurepricemodel']) ? $_POST['setupprocedurepricemodel'] : '');
	$dept = 'OPD';
	Global $instcode;
	
	// 28 NOV 2023 JOSEPH ADORBOE  
	switch ($setupprocedurepricemodel)
	{

		// 28 NOV 2023 JOSPH ADORBOE
		case 'editprocedureprices':
		...
		case 'disableprocedureprice':
			...
			if($preventduplicate == '1'){
				if(empty($ekey)  ){
					...
					$action = "Disable Procedure Price";									
					$result = $engine->getresults($sqlresults,$item,$action);
					$re = explode(',', $result);
					$status = $re[0];					
					$msg = $re[1];
		...
		case 'procedurechangeprices':
			...
			if($preventduplicate == '1'){
				if(empty($ekey) || empty($cashprice) ){
					...
					$cash = 'CASH';
					$sqlresults = $proceduresetuptable->procedurechangeprice($ekey,$cashprice,$alternateprice,$partnerprice,$dollarprice,$insuranceprice,$instcode);									
					$action = "Change Procedure Price";
					if($sqlresults =='2'){							
