<?php
	...
	$setuplabspricemodel = htmlspecialchars(isset($_POST['setuplabspricemodel']) ? $_POST['setuplabspricemodel'] : '');
	$dept = 'OPD';
	Global $instcode;
	
	// 29 NOV 2023 JOSEPH ADORBOE  
	switch ($setuplabspricemodel)
	{
		...
		case 'labschangeprices':
			...
			if($preventduplicate == '1'){
				if(empty($ekey) || empty($cashprice) ){
					...
					$cash = 'CASH';
					$sqlresults = $labtesttable->updatelabsprices($ekey,$cashprice,$alternateprice,$insuranceprice,$partnerprice,$dollarprice,$currentuser,$currentusercode,$instcode);									
					$action = "Change Labs Price";
					if($sqlresults =='2'){							
						$sqlresults = $pricingtable->setcashprices($itemcode,$itemname,$cashprice,$alternateprice,$dollarprice,$partnerprice,$cashpricecode,$category,$cash,$cashschemecode,$cashpaymentmethod,$cashpaymentmethodcode,$currentusercode,$currentuser,$instcode);
