<?php	
	...
	$pendingprocedurescount = $patientproceduretable->getpendingprocedurereport($currentusercode,$instcode);
	if($currentuserlevel == 1){ 
		$recordscount = $patientsServiceRequesttable->getrecordscounts($instcode,$currentshiftcode); 
	...
	if($currentuserlevel == 4 || $currentuserlevel == 5 || $currentuserlevel== 38){ 
		$fp =  $patientnursefollowuptable->getnursefollowupscount($day,$instcode); 
		$newreview = $patientreviewtable->getnewreviewfollowupscount($day,$instcode);
		$newreferals = $patientreferaltable->getcounttodayreferals($day,$instcode);
	} 
		...
		<div class="app-sidebar__overlay" data-toggle="sidebar"></div>
				<aside class="app-sidebar toggle-sidebar">
					<div class="app-sidebar__user pb-0">
						<?php  if($useraccesslevel == 100){ ?>
						<a href="#" title="Switch" data-toggle="modal" data-target="#largeModalswitch"><i class="ti-move" data-toggle="tooltip" title="ti-move"></i> Switch User Level</a>						
						<br />
					...
					<?php 
						if($currentuserlevel == 1){
							require_once (MENUFOLDER.'menu_record.php');  
						} else if($currentuserlevel == 2){
							require_once 'menu_billing.php';
						} else if($currentuserlevel == 3){
							require_once 'menu_cashier.php';
						} else if($currentuserlevel == 4){
							require_once 'menu_billing.php';
						} else if($currentuserlevel == 5){
							require_once 'menu_billing.php';
						} else if($currentuserlevel == 6){
							require_once 'menu_labs.php';
						} else if($currentuserlevel == 13){
							require_once 'menu_radiology.php';
						}
					?>					
				</aside>				
				<!--sidemenu end-->
				<div class="app-content  my-3 my-md-5 toggle-content">
					<div class="side-app">	
						<div id="largeModalswitch" class="modal fade">
						<div class="modal-dialog modal-lg" role="document">
							<div class="modal-content ">
								<div class="modal-header pd-x-20">
									<h6 class="modal-title">Switch  Userlevel </h6>
									...
									<form action="#" method="post">
										<input type="hidden" name="form_number" id="form_number" value="83" />
										<input type="hidden" name="form_key" id="form_key" value="<?php echo md5(microtime());?>" />										
										<div class="row">	
											<div class="col-md-6">
												<div class="form-group">
												<label class="form-label">Userlevel*</label>
												<select name='theuserlevel' class="form-control" required >
												<?php $lov->getuserlevellov(); ?>
												<option value="49@@@OCTOPUS ADMIN"> OCTOPUS ADMIN </option>
												<option value="40@@@HOSPITAL ADMIN"> HOSPITAL ADMIN </option>
												</select>
									...
									<div class="modal-footer">
										<button type="submit" class="btn btn-success" name="setupusermodel" value="switchuserlevel"> <i class="ti-save" data-toggle="tooltip" title="save"></i> Save </button>
										<button type="button" class="btn btn-secondary" data-dismiss="modal"> <i class="ti-close" data-toggle="tooltip" title="Close"></i> Close</button>
									</div>
								</form>
							</div>
						</div><!-- modal-dialog -->
