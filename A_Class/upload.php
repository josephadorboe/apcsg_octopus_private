...
public function uploadAfile($file,$neu_name="-1",$size="-1"){
	$format_types = array("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","application/vnd.ms-excel");
	$uploadit = $uploadpass_size = $uploadpass_type = true;
	$target_dir = SPATH_FILES;
 ...
 public function uploadAttachement($file){
	...
	if(in_array($_type_,$ext)){
        ...
        if(@move_uploaded_file($_tmp_name_,DOC_FOLDERS.$neu_name))
	{
		return $neu_name;
...
public function uploaddoc($file,$neu_name="-1",$size="-1"){
	$format_types = array('image/pjpeg','image/jpeg','image/jpg','image/png','image/x-png','image/gif');
	$uploadit = $uploadpass_size = $uploadpass_type = true;
	$target_dir = ATTACH_CARDREQUEST;
	$neu_name = ($neu_name == "-1")?basename($_FILES[$file]["name"]):$neu_name;
	$target_file = $target_dir.DS.$neu_name;
	//check file exits
	if (!file_exists($target_file)) {
 ...
 public function uploadcardfund($file,$neu_name="-1",$size="-1"){
	$format_types = array('image/pjpeg','image/jpeg','image/jpg','image/png','image/x-png','image/gif');
	$uploadit = $uploadpass_size = $uploadpass_type = true;
	$target_dir = ATTACH_CARDFUND;
	$neu_name = ($neu_name == "-1")?basename($_FILES[$file]["name"]):$neu_name;
	$target_file = $target_dir.DS.$neu_name;
	//check file exits
	if (!file_exists($target_file)) {
	...
	//$target_file = $target_dir.DS.$neu_name;
        ...
        $filename= md5(json_encode($_FILES['picturename']['name'])) . date("his")."." . $ext[1];
			//$target_file = $target_dir.DS.$neu_name;
	...
	public function painty($str) //html2img
{
/*
	Script Name: Painty
